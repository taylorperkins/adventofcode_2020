...
timestamp such that the first bus ID departs at that time and each subsequent listed bus ID departs at
that subsequent minute.
...
def clean_input(input_):
    ...
    earliest_time, ids = input_[0], input_[1]
    earliest_time = int(earliest_time)
    ids = [int(v) for v in ids.split(",") if v != "x"]

    return earliest_time, ids


def get_next_bus(earlies_time, ids):
    shortest_wait = earlies_time
    bus_id = ...

    for id in ids:
        wait = id - (earlies_time % id)
        if wait < shortest_wait:
            shortest_wait = wait
            bus_id = id

    return bus_id * shortest_wait


def main(input_):
    earliest_time, ids = clean_input(input_)
    print(get_next_bus(earliest_time, ids))
