...
route directly to safety, it produced extremely circuitous instructions.
...
class Navigator:
    def __init__(self):
        ...
        self.facing = self.cardinal_directions[0]
        ...
        self.actions = {
            ...
            "L": self.turn,
            "R": self.turn,
            "F": self.move_facing_direction
        }

    def run(self, instructions):
        for instruction in instructions:
            action, units = instruction[0], instruction[1:]
            units = int(units)

            self.perform_action(action, units)

        return abs(self.pos.x)+abs(self.pos.y)

    def perform_action(self, action: str, units: int):
        func = self.actions[action]
        func(action, units)

    def move(self, cardinal: str, units: int):
        if cardinal == "N":
            self.pos = Coord(x=self.pos.x, y=self.pos.y+units)
        if cardinal == "S":
            self.pos = Coord(x=self.pos.x, y=self.pos.y-units)
        if cardinal == "E":
            self.pos = Coord(x=self.pos.x+units, y=self.pos.y)
        if cardinal == "W":
            self.pos = Coord(x=self.pos.x-units, y=self.pos.y)

    def move_facing_direction(self, _: str, unit: int):
        self.move(self.facing, unit)

    def turn(self, direction: str, units: int):
        while self.cardinal_directions[0] != self.facing:
            self.cardinal_directions.rotate(1)

        degrees = units // 90
        degrees = -degrees if direction == "L" else degrees

        self.cardinal_directions.rotate(degrees)
        self.facing = self.cardinal_directions[0]


def main():
    navigator = Navigator()
    input_gen = (line for line in input_.split("\n"))
    print(navigator.run(instructions=input_gen))
