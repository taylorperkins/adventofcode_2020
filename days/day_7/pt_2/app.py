...
In fact, it looks like you'll even have time to grab some food: all flights are currently
...
If you wanted to carry it in at least one other bag, how many different bag colors would be valid
for the outermost bag? (In other words: how many colors can, eventually, contain at least one shiny gold bag?)
...
Of course, the actual rules have a small chance of going several levels deeper than this example; be sure
to count all of the bags, even if the nesting becomes topologically impractical!
...
class Bag:
    ...
    def __init__(self, color):
        self.color = color
        self.parents: Dict[Bag, int] = dict()
        self.children: List[Bag] = []
...
def parse_line(line: str):
    ...
    parent = parent_ptrn.match(line).group("parent")
    parent_bag = get_bag(color=parent)

    for number, child in children_ptrn.findall(line):
        child_bag = get_bag(color=child)

        parent_bag.children.append(child_bag)
        child_bag.parents[parent_bag] = int(number)

    bags[parent] = parent_bag


def create_bags(input_: str):
...
@cache_counts
def count_bags_for_color(color: str):
    bag = bags[color]
    total_bags = 1
    for child in bag.children:
        total_bags += count_bags_for_color(child.color)*child.parents[bag]
    return total_bags
...
def main(input_: str):
    ...
    # minus one for our own bag
    print(count_bags_for_color(our_bag)-1)


if __name__ == '__main__':
    main("""plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
