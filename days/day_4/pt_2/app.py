...
passports have all required fields.
...
Pole Credentials, not a passport at all!
...
passports with invalid data are getting through.
...
class HeightMeasurementUnitEnum(str, Enum):
    centimeters = "cm"
    inches = "in"
...
class Passport(BaseModel):
    byr: int = Field(
        ...,
        ge=1920, le=2002,
        description="Birth Year - four digits; at least 1920 and at most 2002."
    )

    iyr: int = Field(
        ...,
        ge=2010, le=2020,
        description="Issue Year - four digits; at least 2010 and at most 2020."
    )

    eyr: int = Field(
        ...,
        ge=2020, le=2030,
        description="Expiration Year - four digits; at least 2020 and at most 2030."
    )

    hgt: str = Field(
        ...,
        description="Height - a number followed by either cm or in: "
    ...
    ecl: EyeColorEnum = Field(
        ...,
        description="Eye Color - exactly one of: amb blu brn gry grn hzl oth."
    )
    ...
    cid: str = Field(
        None,
        description="Country ID - ignored, missing or not."
    )

    @validator("hgt")
    def _hgt(cls, v):
        measure, unit = int(v[:-2]), HeightMeasurementUnitEnum(v[-2:])

        if (unit == HeightMeasurementUnitEnum.centimeters and not (150 <= measure <= 193)) \
                or (unit == HeightMeasurementUnitEnum.inches and not (59 <= measure <= 76)):
            raise ValueError(f"{v} is not a valid height")

        return v


def split_records(input_: str) -> Generator[None, str, None]:
    yield from re.split(record_delimiter_ptrn, input_)


def normalize_records(records) -> Generator[None, Dict, None]:
    for record in records:
        record_ = dict()
        for kv in record.replace("\n", " ").split(" "):
            key, value = kv.split(":")
            record_[key] = value
...
if __name__ == '__main__':
    print(main("""ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm
