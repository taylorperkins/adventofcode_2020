...
Any given adapter can take an input 1, 2, or 3 jolts lower than its rating and still produce its rated
output joltage.
...
The charging outlet has an effective rating of 0 jolts, so the only adapters that could connect to
    it directly would need to have a joltage rating of 1, 2, or 3 jolts.
...
def main(input_: str):
    ...
    current_adapter = charging_outlet_rating

    while adapters:
        adapter = adapters.pop(0)
        differences[adapter-current_adapter] += 1
        current_adapter = adapter

    print(differences, differences[3]*differences[1])
