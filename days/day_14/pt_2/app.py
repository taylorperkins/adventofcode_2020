...
Immediately before a value is written to memory, each bit in the bitmask modifies the corresponding
...
In practice, this means the floating bits will take on all possible values, potentially causing many
...
are floating.
Floating bits take on every possible combination of values; with two floating bits, four actual
...
def apply_masks(indices, v):
    try:
        ...
        for i in ("0", "1"):
            v[ind] = i
            yield from apply_masks(rest, v)


def mask_gen(mask: str):
    replacements, floating = [], []

    for ind, v in enumerate(mask):
        if v == "1":
            replacements.append(ind)
        elif v == "X":
            floating.append(ind)
    ...
    while True:
        address = yield masks
        address = list(to_bit(int(address)))

        for ind in replacements:
            address[ind] = "1"

        masks = (int("".join(a), 2) for a in apply_masks(floating, address))
...
def main():
    ...
    for line in program_input.splitlines():
        if line[:3] == "mem":
            ...
            for address in mask.send(address):
...
if __name__ == '__main__':
    # result = mask(indices=[1, 2, 4], v=list("00000000000000000000000000000001X0XX"))
