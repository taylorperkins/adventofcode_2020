...
def get_surrounding_areas(coord, shape):
    coords = (
        (coord[0]-1, coord[1]-1,),
        (coord[0]-1, coord[1],),
        (coord[0]-1, coord[1]+1,),
    ...
    return [
        ...
        if 0 <= coord[0] < shape[0] and 0 <= coord[1] < shape[1]
    ]
...
def update_layout(
    ...
    for i, row in enumerate(layout):
        ...
        for j, char in enumerate(row):
            surrounding_areas = get_surrounding_areas((i, j), shape)
            current_area = layout[i][j]

            surrounding_area_occupied = False
            surrounding_area_occupied_count = 0
            for row_idx, col_idx in surrounding_areas:
            ...
            if current_area == "L" and not surrounding_area_occupied:
                new_row += "#"
                seats_changed += 1
            elif current_area == "#" and surrounding_area_occupied_count >= 4:
                new_row += "L"
                seats_changed += 1
            else:
